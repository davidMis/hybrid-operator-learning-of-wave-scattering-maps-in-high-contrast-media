...
from helmholtz_hybrid.data import ContrastDataset, HybridDataset, ScOTDatasetWrapper, load_task_dataset
...
def evaluate_checkpoint(request: EvaluationRequest) -> EvaluationResult:
    ...
    if request.model_type == "fno":
        ...
        expected, actual = evaluate_hybrid(
    ...
    per_sample = relative_l2_per_sample(expected, actual)
    metrics = {
...
def relative_l2_per_sample(expected: np.ndarray, actual: np.ndarray) -> np.ndarray:
    ...
    numerator = complex_L2_norm(torch.as_tensor(diff))
    denominator = complex_L2_norm(torch.as_tensor(expected))
    return (numerator / (denominator + 1e-8)).numpy()
...
def evaluate_fno(
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc=progress_label, unit="batch", dynamic_ncols=True):
            chunks.append(model(batch["x"].to(device)).cpu().numpy())

    actual = np.concatenate(chunks, axis=0)
    return _expected_and_full_pressure(dataset, actual)
...
def evaluate_scot(
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc=progress_label, unit="batch", dynamic_ncols=True):
            ...
            chunks.append(scot_predictions(output).cpu().numpy())
...
def evaluate_hybrid(
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc=progress_label, unit="batch", dynamic_ncols=True):
            ...
            chunks.append((pressure_smooth + pressure_delta).cpu().numpy())

    return dataset.pressure_sharp, np.concatenate(chunks, axis=0)


def _expected_and_full_pressure(dataset, actual: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    if isinstance(dataset, ContrastDataset):
        return dataset.pressure_delta, actual
    if hasattr(dataset, "pressure_smooth"):
        return dataset.pressure_smooth, actual
    if hasattr(dataset, "pressure_sharp"):
        return dataset.pressure_sharp, actual
